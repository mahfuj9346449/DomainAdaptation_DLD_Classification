...
class DA_Model(object):
    ...
    def build_model(self):
        ...
        # feature consistant in scale 1
        self.tar_fea_cons_sc1 = self.featureConsistencyLayer([self.g2_target_feature[0], self.g2_target_feature[1]],
                                                             ksize=3,
                                                             scope_name='FCL1',
                                                             is_training=self.is_training,
                                                             adjustScale=False)

        self.D_src_fea_sc1 = self.Discriminator(self.g2_source_feature[1],
                                                ksize=3,
                                                scope_name='D1',
                                                is_training=self.is_training,
                                                keep_rate=self.keep_rate)
        self.D_tar_fea_sc1 = self.Discriminator(self.g2_target_feature[1],
                                                ksize=3,
                                                scope_name='D1',
                                                is_training=self.is_training,
                                                keep_rate=self.keep_rate)

        # feature consistant in scale 2
        self.tar_fea_cons_sc2 = self.featureConsistencyLayer([self.g2_target_feature[1], self.g2_target_feature[2]],
                                                             ksize=3,
                                                             scope_name='FCL2',
                                                             is_training=self.is_training,
                                                             adjustScale=True)

        self.D_src_fea_sc2 = self.Discriminator(self.g2_source_feature[2],
                                                ksize=3,
                                                scope_name='D2',
                                                is_training=self.is_training,
                                                keep_rate=self.keep_rate)
        self.D_tar_fea_sc2 = self.Discriminator(self.g2_target_feature[2],
                                                ksize=3,
                                                scope_name='D2',
                                                is_training=self.is_training,
                                                keep_rate=self.keep_rate)
        ...
        with tf.variable_scope('loss'):
            ...
            self.FCL_loss1 = tf.losses.absolute_difference(self.g2_target_feature[0],
                                                           self.tar_fea_cons_sc1)

            self.FCL_loss2 = tf.losses.absolute_difference(self.g2_target_feature[1],
                                                           self.tar_fea_cons_sc2)

            self.FCL_loss3 = tf.losses.absolute_difference(self.g2_target_feature[2],
                                                           self.tar_fea_cons_sc3)

            self.FCL_loss = self.FCL_loss1 + self.FCL_loss2 + self.FCL_loss3

            # generator losses
            self.g2_target_gloss1 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
                logits=self.D_tar_fea_sc1,
                labels=tf.ones_like(self.D_tar_fea_sc1)))

            self.g2_target_gloss2 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
                logits=self.D_tar_fea_sc2,
                labels=tf.ones_like(self.D_tar_fea_sc2)))
            ...
            self.g2_source_dloss1 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
                logits=self.D_src_fea_sc1,
                labels=tf.ones_like(self.D_src_fea_sc1)))
            self.g2_target_dloss1 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
                logits=self.D_tar_fea_sc1,
                labels=tf.zeros_like(self.D_tar_fea_sc1)))

            self.g2_source_dloss2 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
                logits=self.D_src_fea_sc2,
                labels=tf.ones_like(self.D_src_fea_sc2)))
            self.g2_target_dloss2 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
                logits=self.D_tar_fea_sc2,
                labels=tf.zeros_like(self.D_tar_fea_sc2)))
            ...
            self.d1_loss = self.g2_source_dloss1 + self.g2_target_dloss1
            self.d2_loss = self.g2_source_dloss2 + self.g2_target_dloss2
            ...
            tf.summary.scalar('g_loss1', self.g2_target_gloss1)
            tf.summary.scalar('g_loss2', self.g2_target_gloss2)
            tf.summary.scalar('g_loss3', self.g2_target_gloss3)
            tf.summary.scalar('g_loss', self.g_loss)
            tf.summary.scalar('d_loss1', self.d1_loss)
            tf.summary.scalar('d_loss2', self.d2_loss)
            tf.summary.scalar('d_loss3', self.d3_loss)
            tf.summary.scalar('d_loss', self.d_loss)
            tf.summary.scalar('FCL1_loss', self.FCL_loss1)
            tf.summary.scalar('FCL2_loss', self.FCL_loss2)
        ...
        with tf.variable_scope('optimization_variables'):
            ...
            self.FCL1_var = [var for var in self.t_var if 'FCL1' in var.name]
            self.FCL2_var = [var for var in self.t_var if 'FCL2' in var.name]
            self.FCL3_var = [var for var in self.t_var if 'FCL3' in var.name]
            self.FCL_var = self.FCL1_var + self.FCL2_var + self.FCL3_var
            ...
            self.d_var = self.d1_var + self.d2_var + self.d3_var
