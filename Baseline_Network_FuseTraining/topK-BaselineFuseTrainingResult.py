...
def getInformation(file):
    ...
    with open(file, 'r') as f:
        ...
        for l in range(len(lines)):
            ...
            if _line_information.startswith('test mode source'):
            ...
            if _line_information.startswith('test mode target'):
        ...
        for e, v, s_t_a, s_t_f, t_t_a, t_t_f in zip(epoch_list, val_acc_list, src_tst_acc_list, src_tst_fscore_list,
                                                    tar_tst_acc_list, tar_tst_fscore_list):
            summarized_list.append([e, v, s_t_a, s_t_f, t_t_a, t_t_f])

    return summarized_list


def sortList(summarized_list):
    sorted_list = sorted(summarized_list, key=lambda s: s[1], reverse=True)
...
def showResults(sorted_list, count):
    for i in range(count):
        ...
        print(
            ...
            'Target Test Accuracy [%.4f], Target Test FScore [%.4f]' % (
                sorted_list[i][0], sorted_list[i][1], sorted_list[i][2], sorted_list[i][3], sorted_list[i][4],
                sorted_list[i][5]))
    print()
...
def main():
    top_k = 20
    file_dir = 'D:/2/experimental_logs/Baseline_FuseTraining/'
