...
class ResNet(object):
    def __init__(self, model_name, sess, train_data, val_data, tst_data, epoch, restore_epoch, reload_path, num_class,
                 ksize, out_channel1, out_channel2, out_channel3, learning_rate, batch_size, img_height, img_width,
                 train_phase):
    ...
    def saveConfiguration(self):
        FineTuning_utils.save2file('epoch : %d' % self.eps, self.ckptDir, self.model)
        FineTuning_utils.save2file('restore epoch : %d' % self.res_eps, self.ckptDir, self.model)
        FineTuning_utils.save2file('model : %s' % self.model, self.ckptDir, self.model)
        FineTuning_utils.save2file('ksize : %d' % self.k, self.ckptDir, self.model)
        FineTuning_utils.save2file('out channel 1 : %d' % self.oc1, self.ckptDir, self.model)
        FineTuning_utils.save2file('out channel 2 : %d' % self.oc2, self.ckptDir, self.model)
        FineTuning_utils.save2file('out channel 3 : %d' % self.oc3, self.ckptDir, self.model)
        FineTuning_utils.save2file('learning rate : %g' % self.lr, self.ckptDir, self.model)
        FineTuning_utils.save2file('batch size : %d' % self.bs, self.ckptDir, self.model)
        FineTuning_utils.save2file('image height : %d' % self.img_h, self.ckptDir, self.model)
        FineTuning_utils.save2file('image width : %d' % self.img_w, self.ckptDir, self.model)
        FineTuning_utils.save2file('num class : %d' % self.num_class, self.ckptDir, self.model)
        FineTuning_utils.save2file('train phase : %s' % self.train_phase, self.ckptDir, self.model)
    ...
    def build_model(self):
        ...
        tf.summary.image('input_x', self.x, max_outputs=3)
    ...
    def test_procedure(self):
        ...
        for j in range(self.num_class):
            log3 += 'category %s test accuracy : %g\n' % (
                FineTuning_utils.pulmonary_category[j], detail_test_accuracy[j])
        log3 = log3[:-1]
    ...
    def train(self):
        ...
        for itr in range(1, self.total_iteration + 1):
            _tr_img_batch, _tr_lab_batch = DA_init.next_batch(image=self.training_data[0], label=self.training_data[1],
                                                              batch_size=self.bs)
