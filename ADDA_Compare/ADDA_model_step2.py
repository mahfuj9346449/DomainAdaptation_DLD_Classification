...
class DA_Model_step2(object):
    def __init__(self, model_name, sess, train_data, val_data, tst_data, epoch, restore_epoch, reloadPath, num_class,
                 learning_rate, batch_size, img_height, img_width, train_phase, step):
    ...
    def build_model(self):
        ...
        with tf.variable_scope('loss'):
            ...
            self.supervised_loss = tf.reduce_mean(
                tf.nn.softmax_cross_entropy_with_logits(logits=self.source_pred, labels=self.y_source))
        ...
        with tf.variable_scope('optimization_variables'):
            ...
            self.src_var = [var for var in self.t_var if 'source_encoder' in var.name]
            self.cla_var = [var for var in self.t_var if 'classifier' in var.name]
            self.tar_var = [var for var in self.t_var if 'target_encoder' in var.name]
            self.dis_var = [var for var in self.t_var if 'Discriminator' in var.name]
